...
import forecastDataJson from "../../../forecasts/forecast_comprehensive_30d.json";
import {
  Download,
  FileText,
  TrendingUp,
...
// Generate forecast table data from JSON
const forecastTableData = forecastDataJson.materials.map((material: any, index: number) => {
  // Calculate average confidence from forecast data (based on prediction interval width)
  const avgForecast = material.forecast.reduce((sum: number, item: any) => sum + item.y_hat, 0) / material.forecast.length;
  const avgLower = material.forecast.reduce((sum: number, item: any) => sum + item.y_lower, 0) / material.forecast.length;
  const avgUpper = material.forecast.reduce((sum: number, item: any) => sum + item.y_upper, 0) / material.forecast.length;
  const intervalWidth = avgUpper - avgLower;
  const confidence = Math.round(Math.max(75, Math.min(98, 100 - (intervalWidth / avgForecast * 100))));

  // Estimate value (assuming $1000 per unit average)
  const avgPricePerUnit = 1000;
  const value = Math.round(material.total_predicted_demand * avgPricePerUnit);

  // Determine category based on material name
  const materialNameLower = material.material_name.toLowerCase();
  let category = "General";
  if (materialNameLower.includes('steel') || materialNameLower.includes('rods') || materialNameLower.includes('aggregate')) {
    category = "Structural";
  } else if (materialNameLower.includes('cement') || materialNameLower.includes('concrete') || materialNameLower.includes('bricks')) {
    category = "Construction";
  } else if (materialNameLower.includes('wire') || materialNameLower.includes('copper')) {
    category = "Electrical";
  } else if (materialNameLower.includes('paint') || materialNameLower.includes('tiles') || materialNameLower.includes('glass')) {
    category = "Finishing";
  } else if (materialNameLower.includes('sand')) {
    category = "Raw Material";
  }

  // Determine status based on lead time and demand
  let status = "normal";
  let note = "";
  if (material.lead_time_days > 30) {
    status = "long-lead";
    note = "Long lead time";
  } else if (material.lead_time_days < 5) {
    status = "high-priority";
    note = "Short lead time - high priority";
  } else if (material.total_predicted_demand > 400) {
    status = "high-priority";
    note = "High demand";
  ...
  const materialDisplay = material.material_name
    .replace(/_/g, ' ')
    .replace(/\b\w/g, (l: string) => l.toUpperCase());

  // Get forecast month (first date in forecast)
  const forecastMonth = new Date(material.forecast[0].date).toLocaleDateString('en-US', {
    month: 'short',
    year: 'numeric'
  });
  ...
  return {
    ...
    category: category,
    month: forecastMonth,
    demand: Math.round(material.total_predicted_demand),
    unit: material.unit.replace(/_/g, ' '),
...
const Forecast = () => {
  ...
  // Get selected material data or use aggregated forecast
  const getSelectedForecastData = () => {
    if (selectedMaterial === "all") {
      return forecastDataJson.aggregated_forecast;
    }
    const material = forecastDataJson.materials.find(
      (m: any) => m.material_name === selectedMaterial
    );
    return material ? material.forecast : forecastDataJson.aggregated_forecast;
  };

  const selectedForecastData = getSelectedForecastData();
  const selectedMaterialData = selectedMaterial === "all"
    ? null
    : forecastDataJson.materials.find((m: any) => m.material_name === selectedMaterial);

  // Transform ML model JSON data to chart format
  const mlChartData = selectedForecastData.map((item: any) => ({
  ...
  selectedForecastData.forEach((item: any) => {
    const monthKey = new Date(item.date).toLocaleDateString('en-US', { month: 'short', year: 'numeric' });
  ...
  const mlMonthlyChartData = Object.entries(monthlyData).map(([month, items]) => {
    const totalForecast = items.reduce((sum: number, item: any) => sum + item.y_hat, 0);
    const totalActual = items.reduce((sum: number, item: any) => sum + (item.actual || 0), 0);
    const totalLastYear = items.reduce((sum: number, item: any) => sum + (item.last_year || 0), 0);
    const hasActual = items.some((item: any) => item.actual !== null);
    const hasLastYear = items.some((item: any) => item.last_year !== null);

    return {
      month,
      forecast: Math.round(totalForecast * 100) / 100,
      actual: hasActual ? Math.round(totalActual * 100) / 100 : null,
      lastYear: hasLastYear ? Math.round(totalLastYear * 100) / 100 : null,
    };
  });
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-green-600 dark:text-green-400 mt-1">
                    ✓ ML Model Data • Project: {forecastDataJson.metadata.project_id} • Region: {forecastDataJson.metadata.region} • Supplier: {forecastDataJson.metadata.supplier}
                  </p>
                  <p className="text-xs text-muted-foreground mt-1">
                    Forecast Period: {new Date(forecastDataJson.forecast_start).toLocaleDateString()} - {new Date(forecastDataJson.forecast_end).toLocaleDateString()} ({forecastDataJson.horizon_days} days)
                  </p>
                </div>
              </div>
            </div>
            ...
            <Card>
              <CardHeader>
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    <div>
                      <CardTitle className="text-lg font-semibold">Demand Forecast Trend (ML Model)</CardTitle>
                      <p className="text-sm text-muted-foreground mt-1">
                        {forecastDataJson.horizon_days}-day projection • {selectedMaterial === "all" ? "All Materials" : selectedMaterial}
                        {selectedMaterialData && (
                          <span> • Predicted: {Math.round(selectedMaterialData.total_predicted_demand)} {selectedMaterialData.unit}</span>
                        )}
                        {selectedMaterial === "all" && (
                          <span> • Total: {Math.round(forecastDataJson.total_predicted_demand)} units</span>
                        )}
                      </p>
                    </div>
                    <Select value={selectedMaterial} onValueChange={setSelectedMaterial}>
                      ...
                      <SelectContent>
                        <SelectItem value="all">All Materials (Aggregated)</SelectItem>
                        {forecastDataJson.materials.map((material: any) => (
                          <SelectItem key={material.material_name} value={material.material_name}>
                            {material.material_name.replace(/_/g, ' ').replace(/\b\w/g, (l: string) => l.toUpperCase())}
                          </SelectItem>
                        ))}
                      </SelectContent>
                    </Select>
                  ...
                  {selectedMaterialData && (
                    <div className="flex flex-col gap-1 text-right">
                      <Badge variant="outline" className="gap-1">
                        ...
                        {selectedMaterialData.supplier}
                      </Badge>
                      <span className="text-xs text-muted-foreground">
                        Lead time: {selectedMaterialData.lead_time_days} days
                      </span>
                    </div>
                  )}
                </div>
              ...
              <CardContent>
                <ResponsiveContainer width="100%" height={400}>
                  <ComposedChart data={mlMonthlyChartData}>
                    <defs>
